...
def test_signal_rejects_invalid_runtime_values(action: object, quantity: object) -> None:
    with pytest.raises(ValueError):
        Signal(action, quantity, "invalid input")
...
def test_context_rejects_invalid_position_quantity() -> None:
    with pytest.raises(ValueError, match="position quantities"):
        PortfolioContext(cash=100_000, positions={"NSE_EQ|TEST": 1.5})
...
def test_context_positions_are_read_only() -> None:
    ...
    with pytest.raises(TypeError):
        context.positions["NSE_EQ|TEST"] = 1
...
def test_sma_crossover_buys_on_golden_cross_and_waits_for_window() -> None:
    ...
    assert signals[2] == []
    assert signals[3] == [
        Signal(Action.BUY, quantity=5, reason="SMA golden crossover")
    ]
