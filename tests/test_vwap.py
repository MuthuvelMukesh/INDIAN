...
def test_vwap_reversion_resets_at_each_ist_session() -> None:
    ...
    assert (
        strategy.on_data(candle(datetime(2024, 1, 1, 3, 45, tzinfo=UTC), 100), empty)
        == []
    )
    assert strategy.on_data(
        candle(datetime(2024, 1, 1, 3, 50, tzinfo=UTC), 98), empty
    ) == [Signal(Action.BUY, 2, "VWAP oversold", "NSE_EQ|TEST")]
    assert strategy.on_data(
        candle(datetime(2024, 1, 1, 4, 0, tzinfo=UTC), 102), holding
    ) == [Signal(Action.SELL, 2, "VWAP overbought", "NSE_EQ|TEST")]

    # 03:45 UTC on the next day is a new NSE session and starts a fresh VWAP.
    assert (
        strategy.on_data(candle(datetime(2024, 1, 2, 3, 45, tzinfo=UTC), 100), empty)
        == []
    )
...
def test_vwap_ignores_preopen_prices() -> None:
    ...
    assert (
        strategy.on_data(
            candle(datetime(2024, 1, 1, 3, 0, tzinfo=UTC), 50, 10_000), context
        )
        == []
    )
    assert (
        strategy.on_data(candle(datetime(2024, 1, 1, 3, 45, tzinfo=UTC), 100), context)
        == []
    )
